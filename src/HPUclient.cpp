...
class HostEngineClient {
 public:
  ...
  bool GetDriverVer( const std::string & HLnumber) {
    ...
    Status status = stub_->GetDriverVer(&context, request, &drvver);
    if (!status.ok()) {
      std::cout << "GetDriverVer rpc failed." << std::endl;
      return false;
    }
  ...
  bool ListPCI(const std::string HLnumber)
  {
      ...
      request.set_hlnumber(HLnumber);
      PCIeInfo pcieinfo;
      Status status = stub_->ListPCI(&context, request, &pcieinfo);
      if (!status.ok()) {
	  std::cout << "ListPCI rpc failed." << std::endl; 
	  return false;
      }
      std::cout << "Find PCIe device info:" << 
	      "bus" << pcieinfo.bus() << 
	      "device" << pcieinfo.device() <<
	      "domain" << pcieinfo.domain() <<
	      "deviceId" << pcieinfo.deviceid() << std::endl;
      return true;
...
int main(int argc, char** argv) {
  HostEngineClient Host(grpc::CreateChannel("localhost:50051",
...
class HostEngineClient {
 public:
  HostEngineClient(std::shared_ptr<Channel> channel, const std::string& db)
      : stub_(HostEngine::NewStub(channel)) {
